...
function cockpit_update_machines ()
{
    ...
    var extra_machines = cockpit_settings_get_json("extra-machines");
    var new_extra_machines = [ ];

    function maybe_add_machine (address, dbus_iface) {
        var j, found, m;

        found = false;
        for (j = 0; j < cockpit_machines.length; j++) {
            if (cockpit_machines[j].address == address) {
                found = true;
                break;
            }
        }
        if (!found) {
            cockpit_machines.push({ 'address': address,
                                    'client': new DBusClient(address),
    ...
    for (i = 0; i < machines.length; i++)
        maybe_add_machine (machines[i].Address, machines[i]);

    if (extra_machines) {
        for (i = 0; i < extra_machines.length; i++) {
            if (maybe_add_machine (extra_machines[i], null))
                new_extra_machines.push(extra_machines[i]);
        }
    }

    cockpit_settings_set_json("extra-machines", new_extra_machines);
    ...
    for (j = 0; j < cockpit_machines.length;) {
        ...
        if (!found && extra_machines) {
            for (i = 0; i < extra_machines.length; i++) {
                if (cockpit_machines[j].address == extra_machines[i]) {
                    found = true;
                    break;
                }
            }
...
function cockpit_add_machine (address)
{
    ...
    machines.call('Add', address, function (error) {
        if (error) {
            var extra_machines = cockpit_settings_get_json ("extra-machines");
            if (!extra_machines)
                extra_machines = [ ];
            extra_machines.push(address);
            cockpit_settings_set_json ("extra-machines", extra_machines);
...
{
    if (machine.dbus_iface) {
        machine.dbus_iface.call('Remove', function (error) {
            cockpit_show_unexpected_error (error);
        });
    } else {
        var extra_machines = cockpit_settings_get_json ("extra-machines");
        if (extra_machines) {
            for (var i = 0; i < extra_machines.length; i++) {
                if (extra_machines[i] == machine.address)
                    extra_machines.splice (i, 1);
            }
            cockpit_settings_set_json ("extra-machines", extra_machines);
            cockpit_update_machines ();
        }
...
function cockpit_init_connect_local()
{
    ...
    $(cockpit_dbus_local_client).on('state-change', function () {
        ...
        if (cockpit_dbus_local_client.state == "ready") {
            var extra_machines = cockpit_settings_get_json("extra-machines");
            if (extra_machines && extra_machines.length > 0) {
                var machines = cockpit_dbus_local_client.lookup("/com/redhat/Cockpit/Machines",
                                                                "com.redhat.Cockpit.Machines");
                if (machines) {
                    for (var i = 0; i < extra_machines.length; i++)
                        machines.call('Add', extra_machines[i], function (error) {
                            return;
                        });
                }
            }
